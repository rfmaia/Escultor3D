...
vector<int> Sculptor::getColor(int x, int y, int z){
    vector<int> rgba;
    rgba.push_back(v[x][y][z].r);
    rgba.push_back(v[x][y][z].g);
    rgba.push_back(v[x][y][z].b);
    rgba.push_back(v[x][y][z].a);
    return rgba;
}
...
bool Sculptor::isOn(int x, int y, int z){
    return v[x][y][z].isOn;
}
    ...
    for(int i = xcenter - rx; i <= xcenter + rx; i++){
        for(int j = ycenter - ry; j <= ycenter + ry; j++){
            for(int k = zcenter - rz; k <= zcenter + rz; k++){
                ...
                if ((eq <= 1) && (i >= 0 && i < nx) && (j >= 0 && j < ny) && (k >= 0 && k < nz)){
                    putVoxel(i, j, k);
                }
            }
        }
